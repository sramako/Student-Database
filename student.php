...
<div id="Marks" class="w3-container tab" style="display:none">
  ...
  <p>Score Card.</p> 
   <div>
	<?php
	   //$query="drop table marks_view;"
	   //$res=pg_query($query);
	   $query="select sub1,sub2,sub3,sub4,sub5 from subject;";
	   $res=pg_query($query);
	   $sub=pg_fetch_array($res,null,PGSQL_ASSOC);
		$query="create temp table marks_view as select test,sub1,sub2,sub3,sub4,sub5 from marks where semester = 4 and usn ilike '".$usn."';";
		$res=pg_query($query);
	   $row=pg_fetch_array($res,null,PGSQL_ASSOC);
	   $query2='table marks_view;';
		   $res=pg_query($query2);
		$row1=pg_fetch_array($res,null,PGSQL_ASSOC);
	   $row2=pg_fetch_array($res,null,PGSQL_ASSOC);
	   $row3=pg_fetch_array($res,null,PGSQL_ASSOC);
		$query="update marks_view set sub1 = 0.375*sub1,sub2 = 0.375*sub2,sub3 = 0.375*sub3,sub4 = 0.375*sub4,sub5 = 0.375*sub5 where test like 'T1' or test like 'T2';";
		$res=pg_query($query);
	   $row=pg_fetch_array($res,null,PGSQL_ASSOC);
	   $query="update marks_view
	set
		sub1 = 7*sub1,
		sub2 = 7*sub2,
		sub3 = 7*sub3,
	...
	where
		test like 'ESA';";
	   $res=pg_query($query);
	   $query="create temp view grade_card(sub1,sub2,sub3,sub4,sub5) as select SUM(sub1),SUM(sub2),SUM(sub3),SUM(sub4),SUM(sub5) from marks_view;
";
	   $res=pg_query($query);
	   $query="select * from grade_card;";
		   $res=pg_query($query);
		$row=pg_fetch_array($res,null,PGSQL_ASSOC);
	   $query="select (sub1/10 + sub2/10 + sub3/10 + sub4/10 + sub5/10)/5 as cgp from grade_card;";
	   $res=pg_query($query);
	   $cgp=pg_fetch_array($res,null,PGSQL_ASSOC);
	  ?>
	   <hr>
	   <?php echo ' Marks <br>'; ?>
	   <table border=true padding=5px>
	<tr>
			<th>Test</th>
		   <th><?php echo $sub['sub1']; ?></th>
		   <th><?php echo $sub['sub2']; ?></th>
		   <th><?php echo $sub['sub3']; ?></th>
		   <th><?php echo $sub['sub4']; ?></th>
		   <th><?php echo $sub['sub5']; ?></th>
   </tr>
   <tr>
   ...
   <tr>
	   ...
	   <?php echo '<hr><h2>CGP = ';echo $cgp['cgp'];echo '</h2><hr>'; ?>
  </div>
</div>
...
$query18="select student.name,(sub1 +sub2+sub3+sub4+sub5)/5 as Total from marks,student where test='ESA' and student.usn=marks.usn;";
$query19="select floor(100-(t1+t2))as marks_required_for_S from neww8,neww9 ;";
$q1="create temp view neww8 as select .375*(sub1) as t1  from marks where usn like '".$usn."' and test='T1';";
$q2=" create temp view neww9 as select .375*(sub1) as t2  from marks where usn like '".$usn."' and test='T2';";
